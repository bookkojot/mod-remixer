...
opendir(ddd,$bank);
@bank=map{[$_,-s($_)]}grep{/\.bin$/i}map{$bank."/".$_}readdir(ddd);
%used=();

sub findNearest{
my $target=shift;
my @variants=sort{$a->[1] <=> $b->[1]}map{[$_->[0],abs($target-$_->[1])
+rand()*2000-1000
,$_->[1]]}@bank;
...
$inname=findNearest($sampleLen*2);
print "Reading sample $inname (original ".(($sampleLen*2)).", replacement: ".(-s($inname)).")\n";
